...
def get_logger(exp_name, sweep_id, log_level=0):
    ...
    if not os.path.exists('./experiment/%s/outputs/log/' % exp_name):
        os.makedirs('./experiment/%s/outputs/log/' % exp_name)
    if os.path.exists('./experiment/%s/outputs/log/%s.txt' % (exp_name, str(sweep_id))):
        os.remove('./experiment/%s/outputs/log/%s.txt' % (exp_name, str(sweep_id)))
    fh = logging.FileHandler('./experiment/%s/outputs/log/%s.txt' % (exp_name, str(sweep_id)))
    fh.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s: %(message)s'))
    fh.setLevel(logging.INFO)
    logger.addHandler(fh)
    
    return Logger(logger, './experiment/%s/outputs/tf_log/%s' % (exp_name, str(sweep_id)), log_level)
